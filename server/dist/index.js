...
async function startServer() {
    ...
    const allowedOrigins = new Set([PROD_CLIENT, DEV_CLIENT, ADMIN_CLIENT, ...EXTRA_DEV]);
    const isDev = process.env.NODE_ENV !== "production";
    // Función de PreHandler de Autenticación
    const authenticateAdmin = (req, reply, done) => {
    ...
    fastify.route({
        ...
        handler: async (req, reply) => {
            ...
            try {
                ...
                roomService_1.RoomService.stopCallingCards(roomId);
                const io = fastify.io;
                if (io) {
    ...
    fastify.route({
        ...
        handler: async (req, reply) => {
            try {
                await roomService_1.RoomService.clearAllPlayers(); // Asumiendo que esta función notifica al cliente si es necesario
    ...
    fastify.route({
        ...
        preHandler: [authenticateAdmin],
        handler: async (req, reply) => {
            const list = await roomService_1.RoomService.listRooms();
    ...
    fastify.route({
        ...
        handler: async (req, reply) => {
            ...
            if (!room)
                return reply.code(404).send({ success: false, message: `Sala ${roomId} no encontrada.` });
    ...
    fastify.route({
        ...
        handler: async (req, reply) => {
            ...
            try {
                await roomService_1.RoomService.removePlayer(roomId, playerName);
                // Notificar por socket si está disponible
                const io = fastify.io;
                const updated = await roomService_1.RoomService.getRoom(roomId);
                if (io) {
    ...
    const originValidator = (origin, cb) => {
        if (!origin)
            return cb(null, true); // allow non-browser tools / same-origin/no-origin (e.g. mobile native, curl)
    ...
    fastify.ready((err) => {
        ...
        const CLEANUP_INTERVAL = 20000; // cada 20s
        setInterval(async () => {
            try {
                const changes = await roomService_1.RoomService.cleanupStalePlayers(5000); // timeout 5s
                for (const ch of changes) {
                    if (!ch.room) {
                        ...
                        if (ch.room.gameState)
                            io.to(ch.roomId).emit("gameUpdated", ch.room.gameState);
                    }
                }
            }
            catch (e) {
        ...
        io.on("connection", (socket) => {
            ...
            socket.on("claimWin", async (roomId, playerName, payload, callback) => {
                ...
                try {
                    if (!roomId || !playerName) {
                        ...
                        if (typeof callback === 'function')
                            callback({ success: false, error: "invalid_params" });
                        return;
                    }
                    const room = await roomService_1.RoomService.getRoom(roomId);
                    ...
                    if (!board) {
                        ...
                        if (typeof callback === 'function')
                            callback({ success: false, error: "no_board" });
                        return;
                    }
                    ...
                    const validWin = (0, loteria_1.checkWin)(board, markedIndices, mode, firstCard, calledCardIds);
                    ...
                    if (!validWin) {
                        ...
                        if (typeof callback === 'function')
                            callback({ success: false, error: "invalid_pattern" });
                        return;
                    }
                    ...
                    if (room.gameState?.winner) {
                        ...
                        if (typeof callback === 'function')
                            callback({ success: false, alreadyWinner: true });
                        return;
                    }
                    ...
                    const finalRanking = calculateFinalRanking(room.players);
                    room.gameState.finalRanking = finalRanking;
                    console.log(`📊 Ranking calculado:`, finalRanking);
                    // Persistir
                    await roomService_1.RoomService.createOrUpdateRoom(roomId, room);
                    roomService_1.RoomService.stopCallingCards(roomId);
            ...
            socket.on("presence", async ({ roomId, playerName }) => {
                try {
                    if (roomId && playerName)
                        await roomService_1.RoomService.markPlayerActive(roomId, playerName);
                }
                catch (err) {
            ...
            socket.on("joinRoom", async ({ roomId, playerName, playerData }) => {
                try {
                    ...
                    const result = await roomService_1.RoomService.addPlayer(roomId, playerName, playerData);
                    if (!result.added) {
                ...
                if (roomId && playerName) {
                    try {
                        ...
                        if (room?.gameState?.isGameActive) {
                            ...
                            await roomService_1.RoomService.markPlayerOffline(roomId, playerName);
                        }
            ...
            socket.on("updateRoom", async (roomId, payload) => {
                try {
                    if (!roomId || !payload)
                        return;
                    const room = (await roomService_1.RoomService.getRoom(roomId)) || {
                    ...
                    const shouldClearMarks = room.gameState?.winner != null ||
            ...
            socket.on("updateGame", async (payload) => {
                try {
                    if (!payload?.roomId || !payload?.gameState)
                        return;
                    const roomId = payload.roomId;
                    const room = (await roomService_1.RoomService.getRoom(roomId)) || { players: {}, gameState: {} };
                    ...
                    const shouldClearMarks = room.gameState?.winner != null || payload.gameState.isGameActive === false;
                    // Si hay ganador, calcula el ranking con las marcas intactas.
                    if (room.gameState?.winner && room.players) {
                        const finalRanking = calculateFinalRanking(room.players);
                        room.gameState.finalRanking = finalRanking;
            ...
            socket.on("startGameLoop", async (roomId, gameMode) => {
                try {
                    ...
                    const existing = await roomService_1.RoomService.getRoom(roomId);
                    ...
                    const initialRoom = await roomService_1.RoomService.initializeGame(roomId, gameMode);
                    // 2. Inicia el bucle de llamadas automáticas (startCallingCards ya ejecuta la primera llamada)
                    await roomService_1.RoomService.startCallingCards(roomId, io);
            ...
            socket.on("stopGameLoop", async (roomId) => {
                try {
                    console.log(`⏹️ Deteniendo bucle de cartas para sala ${roomId}`);
                    await roomService_1.RoomService.stopCallingCards(roomId);
                    const updated = await roomService_1.RoomService.getRoom(roomId);
                    io.to(roomId).emit("roomUpdated", updated);
                    io.to(roomId).emit("gameUpdated", updated?.gameState);
                }
            ...
            socket.on("startGameCountdown", async (roomId, gameMode, callback) => {
                ...
                try {
                    ...
                    const updatedRoom = await roomService_1.RoomService.initializeGame(roomId, gameMode);

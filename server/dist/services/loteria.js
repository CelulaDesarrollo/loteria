...
exports.WINNING_PATTERNS = exports.CARDS = void 0;
...
function generateBoard() {
    return shuffle(exports.CARDS).slice(0, 16);
}
/**
 ...
 */
function checkWin(board, markedIndices, gameMode = "full", firstCard = null, calledCardIds = []) {
    // Validación básica
    ...
    switch (gameMode) {
        ...
        case "horizontal": {
            const rowPatterns = exports.WINNING_PATTERNS.slice(0, 4);
            const win = rowPatterns.some((pattern) => pattern.every((idx) => markedIndices.includes(idx)));
            console.log(`checkWin(horizontal): patterns=${rowPatterns.length}, win=${win}`);
            return win;
        }
        case "vertical": {
            const colPatterns = exports.WINNING_PATTERNS.slice(4, 8);
            const win = colPatterns.some((pattern) => pattern.every((idx) => markedIndices.includes(idx)));
            console.log(`checkWin(vertical): patterns=${colPatterns.length}, win=${win}`);
            return win;
        }
        case "diagonal": {
            const diagPatterns = exports.WINNING_PATTERNS.slice(8, 10);
            const win = diagPatterns.some((pattern) => pattern.every((idx) => markedIndices.includes(idx)));
            console.log(`checkWin(diagonal): patterns=${diagPatterns.length}, win=${win}`);
            return win;
        }
        case "corners": {
            const corners = exports.WINNING_PATTERNS[10];
            const win = corners.every((idx) => markedIndices.includes(idx));
            console.log(`checkWin(corners): pattern=${corners}, win=${win}`);
            return win;
        }
        case "square": {
            const squarePatterns = exports.WINNING_PATTERNS.slice(11); // [5, 6, 9, 10]
            const win = squarePatterns.some((pattern) => pattern.every((idx) => markedIndices.includes(idx)));
...
function getRestriction(mode, firstCard) {
    switch (mode) {
        ...
        case "corners": // las 4 esquinas del tablero
            return (pos) => (pos.row === 0 || pos.row === 3) &&
                (pos.col === 0 || pos.col === 3);
        /*
